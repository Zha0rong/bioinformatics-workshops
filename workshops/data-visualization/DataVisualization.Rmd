...
top_n_variable_genes <- function(Count_matrix,n) {
  ...
  Results = names(variance[seq(1,n)])
  return(Results)
}
...
CPM_calculation <- function(Counts) {
  ...
  Results = (Counts/colSums(Counts))*10e6
  return(Results)
}
...
PCA_plot <- function(PCA_dimension_reduction_matrix) {
  ...
  require(ggplot2)
  Results=ggplot(data.frame(PCA_dimension_reduction_matrix),aes(x=PC1,y=PC2))+geom_point()
...
PCA_plot_advance <- function(PCA_dimension_reduction_matrix_with_sample_information) {
  ...
  require(ggplot2)
  Results=ggplot(data.frame(PCA_dimension_reduction_matrix_with_sample_information),aes(x=PC1,y=PC2,colour=Condition,shape=Timepoint))+geom_point()
...
volcano.plot <- function(Differential_expression_analysis) {
  ...
  results=ggplot(Differential_expression_analysis,aes(x=log2FoldChange,y=-log10(padj)))+geom_point()
  return(results)
...
volcano.plot_advance <- function(Differential_expression_analysis,lfcthreshold=1,pvalthreshold=0.05) {
  require(ggplot2)
Differential_expression_analysis$color='grey'
for (i in 1:nrow(Differential_expression_analysis)) {
if (Differential_expression_analysis$log2FoldChange[i]>1&Differential_expression_analysis$padj[i]<0.05) {
Differential_expression_analysis$color[i]='red'
}
if (Differential_expression_analysis$log2FoldChange[i]<(-1)&Differential_expression_analysis$padj[i]<0.05) {
Differential_expression_analysis$color[i]='blue'
}
}
Differential_expression_analysis$color=factor(Differential_expression_analysis$color,levels = c('blue','grey','red'))
  results=ggplot(Differential_expression_analysis,aes(x=log2FoldChange,y=-log10(padj),colour=color))+geom_point()+scale_color_manual(values = c('blue','grey','red'))
  return(results)
